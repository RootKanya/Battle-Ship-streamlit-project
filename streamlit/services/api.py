...
# Sesuaikan dengan URL FastAPI kamu
BASE_URL = "http://localhost:8000"

def login_user(username, password):
    # Contoh endpoint: POST /login
    response = requests.post(f"{BASE_URL}/login", json={"username": username, "password": password})
    return response

def register_user(username, password):
    # Contoh endpoint: POST /register
    response = requests.post(f"{BASE_URL}/register", json={"username": username, "password": password})
    return response

def get_leaderboard():
    # Contoh endpoint: GET /leaderboard
...
def make_move(game_id, player, x, y, token):
    # Endpoint untuk menembak koordinat (x, y)
    headers = {"Authorization": f"Bearer {token}"}
...
def get_game_state(game_id, token):
    # Mengambil state terbaru dari board dan skor
    headers = {"Authorization": f"Bearer {token}"}
